...
func WorkOracle(computeProviderId uint, id []byte, startPosition, endPosition uint64, labelSize uint32) ([]byte, error) {
	...
	options := uint32(0)
	output, _, err := gpu.ScryptPositions(computeProviderId, id, salt, startPosition, endPosition, uint8(labelSize), options)
	return output, err
}
...
func WorkOracleOne(cpuProviderID uint, id []byte, position uint64, labelSize uint32) []byte {
	salt := make([]byte, 32) // TODO(moshababo): apply salt
	options := uint32(0)
	output, _, _ := gpu.ScryptPositions(cpuProviderID, id, salt, position, position, uint8(labelSize), options)
	return output
