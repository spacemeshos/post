...
func TestScryptPositions(t *testing.T) {
	...
	for _, p := range providers {
		...
		res, err := ScryptPositions(providerId, id, salt, startPosition, endPosition, hashLenBits, options)
		req.NoError(err)
		req.NotNil(res)
		...
		if prevOutput == nil {
			...
			req.Equal(prevOutput, res.Output, fmt.Sprintf("not equal: provider: %+v, hashLenBits: %v", p, hashLenBits))
		}
	}
}
