...
var (
	...
	postLog      = backendLog.Logger("POST") // global
	rpcServerLog = backendLog.Logger("RPCS")
	diskLog      = backendLog.Logger("DISK")
)

// Initialize package-global logger variables.
func init() {
	signal.UseLogger(postLog)
	initialization.UseLogger(postLog)
	validation.UseLogger(postLog)
	proving.UseLogger(postLog)
	persistence.UseLogger(diskLog)
}
...
func initLogRotator(logFile string, MaxLogFileSize int, MaxLogFiles int) {
	...
	if err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "failed to create file rotator: %v\n", err)
		os.Exit(1)
	}
	logRotator = r
}
